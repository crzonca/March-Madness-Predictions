...
# Model 7: Generalized Poisson Regression with Home Court Advantage
...
def fit_gen_poisson(df, config, verbose=False, use_persisted=True):
    if use_persisted:
        return load_pickle(config.get('output_locations').get('model7_pickle'))
    points_df = df[['Team', 'Opponent', 'Home', 'Points', 'Possessions']]
    ...
    explanatory['Home_Offense'] = df['Home']
    ...
    with open(config.get('output_locations').get('model7_summary'), 'w') as f:
        f.write(str(gen_poisson_model.summary()))
        f.close()
    gen_poisson_model.save(config.get('output_locations').get('model7_pickle'))
...
def get_gen_poisson_model(config, verbose=False):
    ...
    df['Home'] = df.apply(lambda r: 1 if r['Location'] == 'H' else 0, axis=1)

    df = df[['GameDay', 'Team', 'Opponent', 'Points', 'Home', 'FGM', 'FGA', '2FM', '2FA',
...
def get_team_ranking(bracket, config, verbose=True):
    ...
    team_names = set([param.replace('Offense_', '').replace('Defense_', '')
                      for param in model.params.index]) - {'const', 'alpha', 'Home_Offense'}
    ...
    results_df.to_csv(config.get('output_locations').get('model7_rankings'))
...
def fit_possessions(df, config, verbose=False):
    ...
    with open(config.get('output_locations').get('model7_possessions_summary'), 'w') as f:
        f.write(str(results.summary()))
        f.close()
    results.save(config.get('output_locations').get('model7_possessions_pickle'))
...
def create_chance_df(bracket, config, model, possessions_model, use_all=False):
    ...
    scores_df = pd.DataFrame(index=teams, columns=teams)
    ...
    for team1, team2 in itertools.combinations(teams, 2):
        ...
        scores_df.at[team2, team1] = round(dist1.mean(), 1)
        scores_df.at[team1, team2] = round(dist2.mean(), 1)

    chances_df.to_csv(config.get('output_locations').get('model7_chances'))
    scores_df.to_csv(config.get('output_locations').get('model7_scores'))
